...
def import_payrolls(input_payrolls, start, end):
    payrolls = {}	# Dictionary storying lists of Payrolls due on each due date
    due_dates = []
    due_date_capacities = {}
...
def allocate(employees, payrolls, due_date_capacities, due_date, count):
    ...
    for p in all_payrolls:
        ...
        if toggle_do_not_reallocate:
            if payrolls[p].get_do_not_reallocate():
                ...
                for e in all_employees:
                    if employees[e].get_name() == prev_emp_str:
                        y.append(model.NewIntVar(1, 1, "x[%i,%i]" % (e, p)))	# If get_do_not_reallocate() == True, fix possible values of assignment for current employee to that payroll to (1,1) i.e. value must be 1.
            ...
            for e in all_employees:
                y.append(model.NewIntVar(0, 1, "x[%i,%i]" % (e, p)))			# If toggle_do_not_reallocate == False,  allow possible values of assignment to be 0 or 1 for all payrolls.
        print(y)
        x.append(y)
